...
void SoundFxEnvelope::noteOn(void)
{
  IrqAllDisable();
...
void SoundFxEnvelope::noteOff(void)
{
  IrqAllDisable();
...
void SoundFxEnvelope::update(void)
{
  sound_block_t *block;
  uint32_t *     p, *end;
...
bool SoundFxEnvelope::isSustain()
{
  uint8_t current_state = *(volatile uint8_t *)&state;
