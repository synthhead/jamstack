...
bool Clock::update()
{
  ...
  switch(clockMode)
  {
    case ClockMode::INTERNAL_CLOCK: result = shouldStepInternalClock(); break;
    case ClockMode::MIDI_CLOCK: result = shouldStepMidiClock(); break;
    case ClockMode::TRIGGER: result = shouldStepTriggerInput(); break;
  }
  if(result)
  {
    stepCount++;
