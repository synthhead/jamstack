...
FLASHFUNC void usb_init(void)
{
  // TODO: only enable when VBUS detected
  // TODO: return to low power mode when VBUS removed
  // TODO: protect PMU access with MPU
  PMU_REG_3P0 = PMU_REG_3P0_OUTPUT_TRG_ZM(0x0F) | PMU_REG_3P0_BO_OFFSET_ZM(6) | PMU_REG_3P0_ENABLE_LINREG_ZM;
...
#if 1
  if((USBPHY1_PWD
      & (USBPHY_PWD_RXPWDRX_ZM | USBPHY_PWD_RXPWDDIFF_ZM | USBPHY_PWD_RXPWD1PT1_ZM | USBPHY_PWD_RXPWDENV_ZM
         | USBPHY_PWD_TXPWDV2I_ZM | USBPHY_PWD_TXPWDIBIAS_ZM | USBPHY_PWD_TXPWDFS_ZM))
     || (USB1_USBMODE & USB_USBMODE_CM_MASK_ZM))
  {
    // USB controller is turned on from previous use
    // reset needed to turn it off & start from clean slate
    USBPHY1_CTRL_SET = USBPHY_CTRL_SFTRST_ZM; // USBPHY1_CTRL page 3292
    USB1_USBCMD |= USB_USBCMD_RST_ZM;         // reset controller
    int count = 0;
    while(USB1_USBCMD & USB_USBCMD_RST_ZM)
      count++;

    __NVIC_ClearPendingIRQ(USB_OTG1_IRQn);

    USBPHY1_CTRL_CLR = USBPHY_CTRL_SFTRST_ZM; // reset PHY
  ...
  USBPHY1_CTRL_CLR = USBPHY_CTRL_CLKGATE_ZM;
  USBPHY1_PWD      = 0;
  //PRINTF("USBPHY1_PWD=%08lX\n", USBPHY1_PWD);
  //PRINTF("USBPHY1_CTRL=%08lX\n", USBPHY1_CTRL);

  USB1_USBMODE = USB_USBMODE_CM_ZM(2) | USB_USBMODE_SLOM_ZM;
  ...
  USB1_USBINTR = USB_USBINTR_UE_ZM | USB_USBINTR_UEE_ZM | /* USB_USBINTR_PCE | */
                 USB_USBINTR_URE_ZM | USB_USBINTR_SLE_ZM;

  __NVIC_EnableIRQ(USB_OTG1_IRQn);

  //PRINTF("USB1_ENDPTCTRL0=%08lX\n", USB1_ENDPTCTRL0);
  //PRINTF("USB1_ENDPTCTRL1=%08lX\n", USB1_ENDPTCTRL1);
  //PRINTF("USB1_ENDPTCTRL2=%08lX\n", USB1_ENDPTCTRL2);
  //PRINTF("USB1_ENDPTCTRL3=%08lX\n", USB1_ENDPTCTRL3);
  USB1_USBCMD = USB_USBCMD_RS_ZM;
  //transfer_log_head = 0;
  //transfer_log_count = 0;
  //USB1_PORTSC1 |= USB_PORTSC1_PFSC; // force 12 Mbit/sec
}

FLASHFUNC void usb_init_alt(void)
{
  usb_init_serialnumber();
...
void USB_OTG1_DriverIRQHandler(void)
{
  //PRINTF("*");
  ...
  // USB_USBSTS_SLI_ZM - set to 1 when enters a suspend state from an active state
  // USB_USBSTS_SRI_ZM - set at start of frame
  // USB_USBSTS_SRI_ZM - set when USB reset detected

  if(status & USB_USBSTS_UI_ZM)
  {
    //PRINTF("data\n");
    ...
    while(setupstatus)
    {
      ...
      do
      {
        USB1_USBCMD |= USB_USBCMD_SUTW_ZM;
        s.word1 = endpoint_queue_head[0].setup0;
        s.word2 = endpoint_queue_head[0].setup1;
      } while(!(USB1_USBCMD & USB_USBCMD_SUTW_ZM));
      USB1_USBCMD &= ~USB_USBCMD_SUTW_ZM;
  ...
  {                                            // page 3164
    ...
    if((USB1_PORTSC1 & USB_PORTSC1_PR_ZM))
    {
      //PRINTF("reset\n");
    }
    else
  ...
  if(status & USB_USBSTS_TI0_ZM)
  {
    if(usb_timer0_callback != NULL)
      usb_timer0_callback();
  }
  if(status & USB_USBSTS_TI1_ZM)
  {
    if(usb_timer1_callback != NULL)
      usb_timer1_callback();
  }
  if(status & USB_USBSTS_PCI_ZM)
  {
    if(USB1_PORTSC1 & USB_PORTSC1_HSP_ZM)
    {
  ...
  if(status & USB_USBSTS_SLI_ZM)
  { // page 3165
    //PRINTF("suspend\n");
  }
  if(status & USB_USBSTS_UEI_ZM)
  {
    //PRINTF("error\n");
  }
  if((USB1_USBINTR & USB_USBINTR_SRE_ZM) && (status & USB_USBSTS_SRI_ZM))
  {
    //PRINTF("sof %d\n", usb_reboot_timer);
    if(usb_reboot_timer)
    {
...
void usb_start_sof_interrupts(int interface)
{
  ...
  if(!(intr & USB_USBINTR_SRE_ZM))
  {
    USB1_USBSTS  = USB_USBSTS_SRI_ZM; // clear prior SOF before SOF IRQ enable
    USB1_USBINTR = intr | USB_USBINTR_SRE_ZM;
  }
  __enable_irq();
}
...
void usb_stop_sof_interrupts(int interface)
{
  ...
  if(sof_usage == 0)
  {
    USB1_USBINTR &= ~USB_USBINTR_SRE_ZM;
  }
}
...
static void endpoint0_setup(uint64_t setupdata)
{
  ...
  switch(setup.wRequestAndType)
  {
    case 0x0500: // SET_ADDRESS
      ...
      USB1_DEVICEADDR = USB_DEVICEADDR_USBADR(setup.wValue) | USB_DEVICEADDR_USBADRA_ZM;
      return;
...
// configure all other endpoints
    ...
    case 0x0082: // GET_STATUS (endpoint)
      ...
      reply_buffer[0] = 0;
      reply_buffer[1] = 0;
      if((dir && (ctrl & USB_ENDPTCTRL_TXS_ZM)) || (!dir && (ctrl & USB_ENDPTCTRL_RXS_ZM)))
      {
        reply_buffer[0] = 1;
      }
      endpoint0_transmit(reply_buffer, 2, 0);
    ...
    case 0x0302: // SET_FEATURE (endpoint)
      ...
      if(dir)
      {
        *((volatile uint32_t *)&USB1_ENDPTCTRL0 + endpoint) |= USB_ENDPTCTRL_TXS_ZM;
      }
      else
      {
        *((volatile uint32_t *)&USB1_ENDPTCTRL0 + endpoint) |= USB_ENDPTCTRL_RXS_ZM;
      }
      endpoint0_receive(NULL, 0, 0);
      return;
    ...
    case 0x0681:
      for(list = usb_descriptor_list; list->addr != NULL; list++)
      {
        if(setup.wValue == list->wValue && setup.wIndex == list->wIndex)
        {
          ...
          //TODO: investigate here! Do we copy properly?
...
static void endpoint0_complete(void)
{
  ...
  if(setup.word1 == 0x03000921 && setup.word2 == ((4 << 16) | SEREMU_INTERFACE) && endpoint0_buffer[0] == 0xA9
     && endpoint0_buffer[1] == 0x45 && endpoint0_buffer[2] == 0xC2 && endpoint0_buffer[3] == 0x6B)
  {
...
static void schedule_transfer(endpoint_t *endpoint, uint32_t epmask, transfer_t *transfer)
{
  ...
  if(last)
  {
    ...
    do
    {
      USB1_USBCMD |= USB_USBCMD_ATDTW_ZM;
      status = USB1_ENDPTSTATUS;
    } while(!(USB1_USBCMD & USB_USBCMD_ATDTW_ZM) && (ARM_DWT_CYCCNT - cyccnt < 2400));
    //USB1_USBCMD &= ~USB_USBCMD_ATDTW_ZM;
    if(status & epmask)
      goto end;
    //ret |= 0x02;
...
uint32_t usb_transfer_status(const transfer_t *transfer)
{
	...
	while (1) {
		__disable_irq();
		USB1_USBCMD = cmd | USB_USBCMD_ATDTW_ZM;
		status = transfer->status;
		cmd = USB1_USBCMD;
		__enable_irq();
		if (cmd & USB_USBCMD_ATDTW_ZM) return status;
		//if (!(cmd & USB_USBCMD_ATDTW_ZM)) continue;
