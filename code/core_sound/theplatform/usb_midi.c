...
OCMEM static uint8_t txbuffer[TX_SIZE * TX_NUM] __attribute__((aligned(32)));
...
OCMEM static uint8_t rx_buffer[RX_NUM * MIDI_RX_SIZE_480] __attribute__((aligned(32)));
...
void usb_midi_configure(void)
{
  ...
  usb_config_tx(MIDI_TX_ENDPOINT, tx_packet_size, 0, NULL);
  int i;
  for(i = 0; i < RX_NUM; i++)
    rx_queue_transfer(i);
...
static void rx_queue_transfer(int i)
{
  __NVIC_DisableIRQ(USB_OTG1_IRQn);
  ...
  system_common_or_realtime:
    switch(b1)
    {
      ...
      case 0xF2:
        ...
        {
          (*usb_midi_handleSongPosition)(((n >> 16) & 0x7F) | ((n >> 17) & 0x3F80));
        }
        break;
      case 0xF3:
